...
The tool SHALL prompt the user for an SSH key name using a `gum input` field when no name is provided as an argument.
...
- **WHEN** the script is run with a key name as the first argument
...
- **WHEN** the user passes `-n <namespace>` to the script
...
When running interactively (no `-n` flag), the tool SHALL prompt for the namespace using `gum input` pre-filled with the resolved default.
...
- **WHEN** `-n <namespace>` is provided
- **THEN** the namespace prompt SHALL be skipped entirely

### Requirement: Namespace name validation
The namespace SHALL only contain alphanumeric characters, hyphens, and underscores (flat, no slashes). The tool SHALL reject invalid namespaces and exit with code 1.
...
The tool SHALL create `~/.ssh/<namespace>/` with permissions `700` if it does not already exist. This is transparent — it SHALL NOT be treated as a guard or error condition.

#### Scenario: Namespace directory does not exist
- **WHEN** `~/.ssh/<namespace>/` is absent
- **THEN** the tool SHALL create it with `mkdir -p ~/.ssh/<namespace> && chmod 700 ~/.ssh/<namespace>` and continue
...
- **WHEN** the user passes `-C "my comment"` to the script
- **THEN** the `-C` value SHALL be forwarded to `ssh-keygen` as the key comment

#### Scenario: No comment provided
- **WHEN** no `-C` flag is given
- **THEN** `ssh-keygen` SHALL be called without a `-C` argument (using ssh-keygen's default)

### Requirement: Success confirmation
After key generation, the tool SHALL display a styled success message via `gum style` showing the full key path.

#### Scenario: Keys created successfully
- **WHEN** `ssh-keygen` exits with code 0
- **THEN** the tool SHALL print the path to the generated public key using `gum style`
